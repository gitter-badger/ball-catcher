function fig_init
    ...
    uicontrols.sarsa_clear = uicontrol(fig, 'String', 'Sarsa Clear', ...
        'callback', @sarsa_clear, ...
        'Position', [120 400 100 20]);
    
    uicontrols.run_rrt = uicontrol(fig, 'String', 'Run RRT', ...
        'callback', @run_rrt, ...
...
%         'Position', [120 350 100 20]);
    ...
    stats.score.text = uicontrol(stats_panel,'style','text',...
            'String', 'Score',...
            'Position', [10 20 50 20]); % L, B, W, H
    stats.score.edit = uicontrol(stats_panel, 'style', 'edit', 'String', 0, ...
            'Position', [70 20 100 20]); % L B W H
        
    stats.reward.text = uicontrol(stats_panel,'style','text',...
            'String', 'Reward',...
            'Position', [10 50 50 20]); % L, B, W, H
    stats.reward.edit = uicontrol(stats_panel,'style','edit', 'String', 0, ...
            'Position', [70 50 100 20]);
    
    %% Controls panel
    ctrls = {};
    ctrls.panel = uipanel(fig,...
        'units', 'pixels',...
        'Position', [220 450 200 150],...
        'Title', 'Controls', 'FontSize', 11);
    
    ctrls.zoom = uicontrol(ctrls.panel, ...
            'String', 'Zoom',...
            'callback', @zoom_button_press, ...
            'Position', [20 10 70 20]); % L, B, W, H
    ctrls.pan = uicontrol(ctrls.panel, ...
            'String', 'Pan', ...
            'callback', @pan_button_press, ...
            'Position', [110 10 70 20]); % L B W H
        
    ctrls.create_ball = uicontrol(ctrls.panel, 'String', 'Create Ball', ...
            'callback', @create_ball_button_press,...
            'Position', [20 70 70 20]);
    
    ctrls.move_ball = uicontrol(ctrls.panel, 'String', 'Del Balls', ...
            'callback', @ball_del_button_press,...
            'Position', [110 70 70 20]);
        
    ctrls.move_ball = uicontrol(ctrls.panel, 'String', 'Move Ball', ...
            'callback', @ball_move_down,...
